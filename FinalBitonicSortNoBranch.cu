...
__global__ void bitonic_sort_kernel(int *arr, unsigned long long distance, unsigned long long subSequence_size){
  ...
  unsigned long long cond = ((i & subSequence_size)==0 && arr[i]>arr[xorCheck]) || (!arr[i]>arr[xorCheck] && arr[i]<arr[xorCheck]);

  unsigned long long var = cond*xorCheck+!cond*i;
  
  swap_values(&arr[i], &arr[var]);
}
